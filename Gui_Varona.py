...
def main():
    ...
    with st.sidebar:
        ...
        Lp = st.number_input('Insert Lp:   Constant * (10 ** -n)', value=18.86 * (10 ** -6))
        st.write('Lp:', Lp)
        Cp = st.number_input('Insert Cp:   Constant * (10 ** -n)', value=139 * (10 ** -9))
        st.write('Cp:', Cp)
        wp = 1 / np.sqrt(Cp * Lp)
        ...
        Ls = st.number_input('Insert Ls:   Constant * (10 ** -n)', value=18.9 * (10 ** -6))
        st.write('Ls:', Ls)
        Cs = st.number_input('Insert Cs:   Constant * (10 ** -n)', value=103 * (10 ** -9))
        st.write('Cs:', Cs)
        ws = 1 / np.sqrt(Cs * Ls)
