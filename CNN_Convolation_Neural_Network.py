...
class CNN:
    ...
    def split_and_normalize_data(self, data, DataType='Mnist'):
        ...
        self.X_train, self.y_train = self.X_train / 255, self.y_train / 255 #normalize_data
        print('X_train.shape:\n', self.X_train.shape, '\ny_train.shape:\n', self.y_train.shape, '\nX_test.shape:\n', self.X_test.shape, '\ny_test.shape:\n', self.y_test.shape)
        if DataType=='Mnist':
    ...
    def create_neural_network(self):
        ...
        x = tf.keras.layers.Dropout(0.2)(x)
        x = tf.keras.layers.Dense(len(np.unique(self.y_train[0])), activation='softmax')(x)
        self.model = tf.keras.Model(i, x)

    def run_model(self, optimizer='adam', loss='BinaryCrossentropy', batch_size=32, n_epochs=15):
